    ...
    : QCanvasView( canvas, parent, name )
{
    ...
    Moving = 0;   // we are not holding the bone
}
...
{
    if ( Moving )    // if we are holding the bone...
    {
        // ...then move it
...
void SkeletonCanvasView::contentsMouseReleaseEvent( QMouseEvent* )
{
    if ( Moving )    // if we are holding the bone
    {
        ...
        {
            // set zero velocity (we drop the bone)
        ...
        Moving = 0;     // clear the pointer, we are no longer holding the bone
