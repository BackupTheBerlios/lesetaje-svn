/*
  This file is part of "sKeleton" by Sebastian Schneider 
  Copyright (C) 2006 LeSeTaJe.berlios.de
  
  This project reuses code from "bounce" by Gunnstein Lye <gl@ez.no>
  Copyright (C) 2000 eZ systems as, Norway
  (http://www.ez.no)
  bounce is part of a QCanvas tutorial on www.zez.org.
  
  sKeleton and skeleton are free software; you can redistribute it and/or modify
...
Bone::Bone( QCanvasPixmapArray *parray, QCanvas *canvas )
    : QCanvasSprite( parray, canvas )
{
    ...
    Air_resistance = 1;      // air resistance
    Bump_resistance = 0;     // resistance when hitting a wall
    Mass = 1;                     // mass of bone (weight)
}
...
void Bone::setMass(const int m) 
{
	Mass = m;
}


bool Bone::hit( const QPoint &p ) const
{
...
void Bone::advance( int phase )
{

    if ( phase == 0 ) // stuff to be done before we move
    {
        // apply resistance to slow down the bone
